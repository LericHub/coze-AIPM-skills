...
1. **CONTEXT_READ**: [CURRENT_SNAPSHOT_ANALYSIS], [CURRENT_SNAPSHOT_DETAIL], [CURRENT_SNAPSHOT_PROTOTYPING]
   - 从 [CURRENT_SNAPSHOT_ANALYSIS] 读取：项目名称、版本、日期、负责人、项目背景、目标、边界、用户角色、页面清单、业务流程
   - 从 [CURRENT_SNAPSHOT_DETAIL] 读取：B端fullpicture、C端fullpicture、API规划、埋点方案、各页面详细设计
   - 从 [CURRENT_SNAPSHOT_PROTOTYPING] 读取：生成的HTML原型代码文件路径
   - **必须显性调用** Figma MCP 获取设计稿内容：需求概述页面 (@https://www.figma.com/design/EsRhGXGgTpdRMhG05JNWEO/Untitled?node-id=7-11382&m=dev) 和页面需求页面 (@https://www.figma.com/design/EsRhGXGgTpdRMhG05JNWEO/Untitled?node-id=30-296&m=dev)
